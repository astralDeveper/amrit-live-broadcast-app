const DiamondPlan = require("./diamondPlanModel");
...
exports.store = async (req, res) => {
  try {
    ...
    diamondPlan.diamonds = req.body.diamonds;
    diamondPlan.coins = req.body.coins;
    // diamondPlan.rupee = req.body.rupee;
    diamondPlan.tag = req.body.tag;
    // diamondPlan.productKey = req.body.productKey;
...
exports.isTopToggle = async (req, res, next) => {
  try {
    const DiamondPlan = await DiamondPlan.findById(req.query.planId);

    if (!DiamondPlan) {
      return res
        .status(200)
        .json({ status: false, message: "DiamondPlan not found" });
    }
    DiamondPlan.isTop = !DiamondPlan.isTop;

    const topPlan = await DiamondPlan.find({
      isTop: true,
      _id: { $ne: DiamondPlan._id },
    });
    if (topPlan.length > 0 && DiamondPlan.isTop) {
      return res.status(200).json({
        status: false,
        message: "overflow ! only one DiamondPlan allowed in Top   .",
      });
    }
    await DiamondPlan.save();
    ...
    return res
      .status(200)
      .json({ status: true, message: "success", data: DiamondPlan });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
...
exports.createCustomer = async (req, res) => {
  try {
    ...
    let plan;
    if (!req.body.isVip) {
      plan = await DiamondPlan.findById(req.body.planId);
    } else {
